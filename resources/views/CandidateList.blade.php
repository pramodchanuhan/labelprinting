                ...
                <div class="row align-items-center">
                    <div class="col-12">
                        ...
                        <ul class="breadcrumb">
                            ...
                            <li class="breadcrumb-item">All Interviews</li>
                        ...
                        <table class="table table-striped custom-table mb-0 datatable">
                            <thead>
                                <tr>
                                    ...
                                    {{-- <th class="text-center">Assigned HR</th> --}}
                                    <th>Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                ...
                                @foreach ($Assesment_form as $Assesment_form)
                                    <tr>
                                        <td>{{ $loop->index + 1 }}</td>
                                        <td>
                                            <a href="{{ url('salarydetailsform') }}?id={{ $Assesment_form->id }}">
                                                {{ $Assesment_form->candidateName }}
                                        </td>
                                        </a>
                                        ...
                                        <!-- <td class="text-center">
                                            <div class="dropdown action-label">
                                                <select id="stage_{{ $Assesment_form->id }}"
                                                    ...
                                                    @foreach ($stages as $key => $stage)
                                                        <option value="{{ $key }}"
                                                            {{ $Assesment_form->stage == $key ? 'selected' : '' }}>
                                                            {{ $stage }}</option>
                                                    @endforeach
                                            </div>
                                        </td>
                                        <td class="text-center">
                                            <div class="dropdown action-label">
                                                <select id="status_{{ $Assesment_form->id }}"
                                                    class="form-select btn btn-white btn-sm btn-rounded "
                                        ...
                                        <td> {{ $stages->{$Assesment_form->stage} }}</td>
                                        <td> {{ $status->{$Assesment_form->status} }}</td>
                                        {{-- <td class="text-center">
                                            <div class="dropdown action-label">

                                                <select id="assignto_{{ $Assesment_form->id }}"
                                                    class="form-select btn btn-white btn-sm btn-rounded " disabled
                                                    aria-label="Default select example"
                                                    @cannot('iaf asign hr') disabled @endcannot>
                                                    <option value="0"
                                                        {{ $Assesment_form->r1_asignedto == '0' ? 'selected' : '' }}>
                                                        Not
                                                        asigned</option>
                                                    @foreach ($hrs as $hr)
                                                        <option value="{{ $hr->id }}"
                                                            {{ $Assesment_form->r1_asignedto == $hr->id ? 'selected' : '' }}>
                                                            {{ $hr->name }}</option>
                                                    @endforeach
                                            </div>
                                        </td> --}}
                                        <td>
                                            ...
                                            <a class="" data-toggle="modal" href=""
                                                onclick="select_interviewmode({{ $Assesment_form->id }},{{ $Assesment_form->cur_round }})">
                                                <i class="fa fa-telegram m-r-5 btn btn-info btn-sm   btn-flat"></i>
                                            </a>

                                            <a class="" href=""
                                                onclick="event.preventDefault(); delete_iaf({{ $Assesment_form->id }});">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <!-- <a class="dropdown-item" data-toggle="modal" href="#" data-target="#select_interviewmode"><i class="fa fa-comment m-r-5 btn btn-info btn-sm btn-flat"></i>Conduct</a> -->
                                                {{-- <a class="dropdown-item"
                                                    href="{{ url('r1-self-asign') }}?iaf_id={{ $Assesment_form->id }}&user_id={{ auth()->user()->id }}"><i
                                                        class="fa fa-book m-r-5 btn btn-info btn-sm btn-flat"></i> self
                                                    asign</a> --}}
                                                ...
                                                <a class="dropdown-item"
                                                    href="{{ url('salarydetailsform') }}?id={{ $Assesment_form->id }}"><i
                                                ...
                                                @can('iaf asign hr')
                                                    <a class="dropdown-item" href=""
                                                        onclick="event.preventDefault(); update_iaf({{ $Assesment_form->id }},this);"><i
                                                            class="fa fa-edit m-r-5 btn btn-info btn-sm edit btn-flat"></i>
                                                        <span>Edit</span></a>
                                                @endcan

                                                <a class="dropdown-item" href=""
                                                    onclick="event.preventDefault(); delete_iaf({{ $Assesment_form->id }});"><i
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ url('selectinterviewmode') }}" method="POST">
                                ...
                                <select id="cur_round_interviewer" class="select" name="assignto">
                                    ...
                                    @else
                                        <option value="{{ auth()->user()->id }}">
                                            {{ ucfirst(auth()->user()->employee->f_name) . ' ' . ucwords(auth()->user()->employee->l_name) }}
                                        </option>
                                    @endcan
                                </select>
                            </div>
                        ...
                        {{-- <input type="hidden" name="cur_round" value="1"> --}}
        ...
        <script>
            ...
            @can('iaf asign hr')
                ...
                let r1_interviewer = `
                    ...
                    @foreach ($r1_interviewers as $r1_interviewer)
                        <option value="{{ $r1_interviewer->id }}">{{ ucfirst($r1_interviewer->employee->f_name) . ' ' . ucwords($r1_interviewer->employee->l_name) }} </option>
                    @endforeach
                `;
            ...
            @else
                let r1_interviewer = `
                    <option value="0">Not asigned</option>
                    <option value="{{ auth()->user()->id }}">{{ ucfirst(auth()->user()->employee->f_name) . ' ' . ucwords(auth()->user()->employee->l_name) }} </option>
                `;
            @endcan
